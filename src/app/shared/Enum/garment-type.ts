...
export const topGarments = GarmentTypeBySection.filter(item => item.section === GarmentSection.Top).map(garment => garment.type);
export const bottomGarments = GarmentTypeBySection.filter(item => item.section === GarmentSection.Bottom).map(garment => garment.type);
